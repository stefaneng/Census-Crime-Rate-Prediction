---
title: "US Census Crime Rate Modeling - MSG500-MVE190Linear Statistical Models"
author: "Stefan Eng"
output:
  pdf_document:
    keep_tex: yes
...
collinearity between the other variables \cite{isl}. The VIF factors are the computed by regressing all the other explanatory variables on a single variable $x_i$. For each variable $i$, we calculate the VIF by performing $i$ regressions using all other variables to predict $x_i$.
$$
...
$$
Where $R_i^2$ is the $R^2$ when using all other variables in a regression on variable $i$.
...
We can interpret the results from negative binomial similarly to Poisson regression.
